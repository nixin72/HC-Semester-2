...
public class AddProductFrame extends JFrame implements WindowListener
{
	...
	public AddProductFrame()
	{
		...
		cmboxMovieCategory.setModel(new DefaultComboBoxModel(new String[] { "",
				"Unknown", "Family", "Action", "Comedy", "Western", "Drama", "Horror",
				"Sci-Fi" }));
		...
		lblRating.setBounds(0, 246, 73, 16);
		contentPane.add(lblRating);
		
		cmboxRating = new JComboBox();
		cmboxRating.setBounds(85, 243, 145, 22);
		contentPane.add(cmboxRating);
		...
		btnAdd.setBounds(110, 285, 95, 25);
		...
		btnClear.setBounds(272, 285, 95, 25);
	...
	private void btnAdd_actionPerformed(ActionEvent e)
	{
		...
		else
		{
			if (productType == 'M')
			{
				...
				product
						.setCategoryCode(cmboxMovieCategory.getSelectedItem().toString());
				product.setFormatCode(cmboxMovieFormat.getSelectedItem().toString());
				...
				if (productType == 'G')
				{
					...
					product.setCategoryCode(cmboxGameCategory.getSelectedItem()
							.toString());
					product.setFormatCode(cmboxGameFormat.getSelectedItem().toString());
					((Game) product).setPlatformCode(cmboxPlatform.getSelectedItem()
							.toString());
				} // else if (productType == 'G')
			if (inventory.write(product))
				if (productType == 'M')
				{
					JOptionPane.showMessageDialog(
							this,
							"The movie was successfully added with product code "
									+ product.getProductNumber(), "Successful Add",
	...
	private void clearAllFields()
	{
		...
		cmboxRating.setSelectedIndex(0);
	...
	private void enableMovieFields(boolean enabled)
	{
		...
		lblRating.setEnabled(enabled);
		cmboxRating.setEnabled(enabled);
	...
	{
		inventory.closeOutput();
		Product.close();
		JOptionPane.showMessageDialog(this, Movie.getCountMovies() + " movies and "
				+ Game.getCountGames() + " games were added.", "Product count",
				JOptionPane.INFORMATION_MESSAGE);
